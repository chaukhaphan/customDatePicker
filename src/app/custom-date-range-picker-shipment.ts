...
export class CustomDateRangeForShipmentComponent
  implements IDateAngularComp, AfterViewInit, OnDestroy
{
  ...
  onDateChanged(selectedDates: any) {
    ...
    if (
      ...
      this.componentParent.selectedShipmentDateRange = selectedDates;
      this.componentParent.setTimeRangeFilter(selectedDates, 'shipmentDate');
      // if (!!!selectedDates) {
      //   // Remove the filter
      ...
      //     type: 'inRange',
      //     dateFrom: dayjs('2021-12-01'), // you have to use exactly this date format in order for it to work
      //     dateTo: dayjs('2021-12-30'),
      //   });
